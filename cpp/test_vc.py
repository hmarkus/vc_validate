...
def test_invalid_input(logfile):
    ...
    for f in sorted(os.listdir(directory)):
        ...
        out = "ERROR"
        try:
            out = subprocess.check_output('./vc_validate {} {}'.format(os.path.join(directory, f), os.path.join(directory, vc_file)), stderr=subprocess.STDOUT, shell=True)
...
def test_invalid_solution(logfile):
    ...
    for f in sorted(os.listdir(directory)):
        ...
        out = "ERROR"
        try:
            out = subprocess.check_output('./vc_validate {} {}'.format(os.path.join(directory, f), os.path.join(directory, vc_file)), shell=True)
        ...
        logfile.write("#" * 10 + " {}: {}".format(basename, out))
    
    logfile.write(("#" if tc_pass else "x") * 10 + " Passed {}/{} testcase\n".format(passed_count, tc_count))
    logfile.write("\n")
    return passed_count, tc_count
...
def main():
    ...
    with open("test_vc.log", "w") as logfile:
    ...
    print("Log written in test_vc.log")
